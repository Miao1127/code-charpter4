            ...
            for b1 in action:
                ...
                for b2 in action:
                    ...
                    if x2_grid < region_grid_num or x2_grid > rows-region_grid_num or y2_grid < region_grid_num \
                        ...
                        p_value = vf.policy_value(x, y, phi_start, r_value, d, bb, gama, map_data)
...
def move_to_pose(rp_q):
    ...
    while True:
        ...
        for action in b:
            ...
            # 避障动作
            if action == 'a0':
                phi_goal = random.uniform(-np.pi, np.pi)
...
def draw_animation():
    ...
    while True:
        if time.time() - tic > 10:
            np.savetxt('E:/博士论文试验数据/chapter4/swarm1/3/'
    ...
    while True:  # 响应监控端发出的初始化指令
        ...
        if not q.empty():
            ...
            if len(c_msg) == len(usv_list):  # 判断是否收到监控端开启集群指令和集群成员是否都集齐
                ...
                makedir.mkdir('E:/博士论文试验数据/chapter4/swarm1/3/' + str(int(start_time)))
                print("连接到监控端")
                for i in range(5):
