...
def draw_animation(u_q):
    ...
    while True:
        if not u_q.empty():
            ...
            if time.time() - tic > 20:
                dict2json.run(usv_path, 'E:/博士论文试验数据/single/path/' + str(int(start_time)) + '/' +
                              str(int(time.time()) - int(start_time)) + 'usv_path')
                np.savetxt('E:/博士论文试验数据/single/map_data/'
                           + str(int(start_time)) + '/' + str(int(time.time()) - int(start_time))
        ...
        if len(data_serial) == 3 and flag == 0 and data_serial[0] == 'I':
            ...
            if usv_num not in i_usv_list and data_serial[0] == 'I':
                ...
                if len(i_usv_list) == total_usv_num:
                    ...
                    makedir.mkdir('E:/博士论文试验数据/single/path/' + str(int(start_time)))
                    makedir.mkdir('E:/博士论文试验数据/single/map_data/' + str(int(start_time)))
