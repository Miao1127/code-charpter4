...
def draw_animation():
    ...
    while True:
        if time.time() - tic > 20:
            np.savetxt('E:/博士论文试验数据/chapter4/zigzag1/4/'
    ...
    while True:  # 响应监控端发出的初始化指令
        ...
        if not q.empty():
            ...
            if len(c_msg) == len(usv_list):  # 判断是否收到监控端开启集群指令和集群成员是否都集齐
                ...
                makedir.mkdir('E:/博士论文试验数据/chapter4/zigzag1/4/' + str(int(start_time)))
                print("连接到监控端")
                for i in range(5):
...
if __name__ == '__main__':
    ...
    way_point = [[3, 18, 0], [81, 18, np.pi/2], [81, 33, np.pi], [8, 33, np.pi/2], [8, 68, 0], [81, 68, np.pi/2],
                 [81, 83, np.pi], [3, 83, np.pi/2], [3, 83, np.pi/2]]
