...
def draw_animation():
    ...
    while True:
        if time.time() - tic > 20:
            np.savetxt('E:/博士论文试验数据/chapter4/zigzag1/1/'
    ...
    while True:  # 响应监控端发出的初始化指令
        ...
        if not q.empty():
            ...
            if len(c_msg) == len(usv_list):  # 判断是否收到监控端开启集群指令和集群成员是否都集齐
                ...
                makedir.mkdir('E:/博士论文试验数据/chapter4/zigzag1/1/' + str(int(start_time)))
                print("连接到监控端")
                for i in range(5):
...
if __name__ == '__main__':
    ...
    way_point = [[3, 3, 0], [96, 3, np.pi/2], [96, 48, np.pi], [23, 48, np.pi/2], [23, 53, 0], [96, 53, np.pi/2],
                 [96, 98, np.pi], [3, 98, np.pi/2], [3, 98, np.pi/2]]
    cruise_speed = 1                                 # 巡航速度
