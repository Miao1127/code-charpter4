...
# 时间：2020/4/23 22:40
# 作者：苗润龙
# 功能：tcp通信的监控端，用于监控usv编队以zigzag形式进行区域编队搜索，为usv集群局部协同搜索提供对比
# 改进：增加了usv的艏向
...
def tcp_server(server):
    while True:
        ...
        thread = threading.Thread(target=read_msg, args=(connection, address, q1, q2))
        thread.start()
...
def draw_animation(u_q):
    ...
    while True:
        ...
        if time.time() - tic > 20:
            dict2json.run(usv_path, 'E:/博士论文试验数据/chapter4/zigzag2/path/' + str(int(start_time)) + '/' +
        ...
        if len(data_serial) == 3 and flag == 0 and data_serial[0] == 'I':
            ...
            if usv_num not in i_usv_list and data_serial[0] == 'I':
                ...
                if len(i_usv_list) == total_usv_num:
                    ...
                    makedir.mkdir('E:/博士论文试验数据/chapter4/zigzag2/path/' + str(int(start_time)))
