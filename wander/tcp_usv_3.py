...
def move_to_pose(rp_q):
    ...
    while True:
        phi_goal = random.uniform(-1.5, 1.5)
...
def draw_animation():
    ...
    while True:
        if time.time() - tic > 10:
            np.savetxt('E:/博士论文试验数据/chapter4/wander/3/'
    ...
    while True:  # 响应监控端发出的初始化指令
        ...
        if not q.empty():
            ...
            if len(c_msg) == len(usv_list):  # 判断是否收到监控端开启集群指令和集群成员是否都集齐
                ...
                makedir.mkdir('E:/博士论文试验数据/chapter4/wander/3/' + str(int(start_time)))
                print("连接到监控端")
                for i in range(5):
...
if __name__ == '__main__':
    ...
    usv_num = '#3'                                   # 当前usv编号
    start_point = [3, 13, 0]                          # 当前usv出发点位置和艏向角
    cruise_speed = 1                                 # 巡航速度
