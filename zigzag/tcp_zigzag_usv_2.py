...
def draw_animation():
    ...
    while True:
        if time.time() - tic > 20:
            np.savetxt('E:/博士论文试验数据/chapter4/zigzag1/2/'
    ...
    while True:  # 响应监控端发出的初始化指令
        ...
        if not q.empty():
            ...
            if len(c_msg) == len(usv_list):  # 判断是否收到监控端开启集群指令和集群成员是否都集齐
                ...
                makedir.mkdir('E:/博士论文试验数据/chapter4/zigzag1/2/' + str(int(start_time)))
                print("连接到监控端")
                for i in range(5):
...
if __name__ == '__main__':
    ...
    way_point = [[3, 8, 0], [91, 8, np.pi/2], [91, 43, np.pi], [18, 43, np.pi/2], [18, 58, 0], [91, 58, np.pi/2],
                 [91, 93, np.pi], [3, 93, np.pi/2], [3, 93, np.pi/2]]
