    ...
    while True:
        ...
        if not rp_q.empty():
            ...
            while rho > waypoint_threshold:
                ...
                x = x + v * np.cos(phi) * dt + random.uniform(-0.5, 0.5)
                y = y + v * np.sin(phi) * dt + random.uniform(-0.5, 0.5)
                x_diff = x_goal - x
                y_diff = y_goal - y
                rho = np.sqrt(x_diff ** 2 + y_diff ** 2)  # 计算距离
...
def draw_animation():
    print("开启绘制概率图线程")
    global map_data, tic, start_time
    fig = plt.figure("集群"+usv_num+"概率图")
    ax = fig.add_subplot(111)
    while True:
        if time.time() - tic > 20:
            np.savetxt('E:/博士论文试验数据/chapter4/zigzag1/5/'
    ...
    while True:  # 响应监控端发出的初始化指令
        ...
        if not q.empty():
            ...
            if len(c_msg) == len(usv_list):  # 判断是否收到监控端开启集群指令和集群成员是否都集齐
                ...
                makedir.mkdir('E:/博士论文试验数据/chapter4/zigzag1/5/' + str(int(start_time)))
                print("连接到监控端")
                for i in range(5):
...
if __name__ == '__main__':
    ...
    way_point = [[3, 23, 0], [76, 23, np.pi/2], [76, 28, np.pi], [3, 28, np.pi/2], [3, 73, 0], [76, 73, np.pi/2],
                 [76, 78, np.pi], [3, 78, np.pi/2], [3, 78, np.pi/2]]
