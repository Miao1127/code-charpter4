...
def markov_solver(sp_q):
    ...
    while True:
        ...
        if rho <= markov_rho_threshold and usv_info[usv_num][7] == 1:
            ...
            # 随机生成策略
            b[0] = random.choice(action)
            b[1] = random.choice(action)
            b[2] = random.choice(action)
            print("采用动作策略为：%s" % b)
            mutex.acquire()
...
def move_to_pose(rp_q):
    ...
    while True:
        phi_goal = random.uniform(-3.14, 3.14)
        x_goal = x + d_step * np.cos(phi_goal)
        y_goal = y + d_step * np.sin(phi_goal)

        x_goal = round(x_goal, 3)
        y_goal = round(y_goal, 3)

        while 5 > x_goal or x_goal > 95 or 5 > y_goal or y_goal > 95:
            if 5 > x_goal:
                x_goal = x_goal + 1
            if x_goal > 95:
                x_goal = x_goal - 1
...
def draw_animation():
    print("开启绘制概率图线程")
    global map_data, start_time, tic
    fig = plt.figure("集群"+usv_num+"概率图", figsize=(10, 10))

    ax = fig.add_subplot(111)

    while True:
        if time.time() - tic > 10:
            np.savetxt('E:/博士论文试验数据/chapter4/wander/1/'
    ...
    while True:  # 响应监控端发出的初始化指令
        ...
        if not q.empty():
            ...
            if len(c_msg) == len(usv_list):  # 判断是否收到监控端开启集群指令和集群成员是否都集齐
                ...
                makedir.mkdir('E:/博士论文试验数据/chapter4/wander/1/' + str(int(start_time)))
                print("连接到监控端")
                for i in range(5):
...
if __name__ == '__main__':
    ...
    usv_num = '#1'                                   # 当前usv编号
    start_point = [3, 3, 0]                          # 当前usv出发点位置和艏向角
    cruise_speed = 1                                 # 巡航速度
    ...
    r = 3                                           # 传感器有效探测范围
    r_max = 6                                        # 传感器最大探测范围，滚动窗口默认为3倍此长度
    r_value = 10                                     # 信息奖励计算范围
    d_step = 15                                      # USV的航行步长
    d = 1                                            # 栅格尺寸
    Kp_alpha = 40                                   # 角速度控制参数
